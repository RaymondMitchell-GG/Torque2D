...
{      
   ...
   for (%i = 1; %i <= PixelPainter.NumberOfLevels; %i++)
   {  
      %spriteobject = new Sprite();    
      %this.GallerySprites[%i] = %spriteobject;
                  
      if(%i <= PixelPainter.savedata.highestComplete)
      {
         %imagename = "PixelPainter:LevelComplete" @ %i;
...
function galleryViewer::onTouchDragged(%this, %modifier, %worldPosition, %clicks)
{
   ...
   if (mAbs(%this.currentDelta.x) > 20)
   {
      ...
      if (%this.currentDelta.x > 0)
      {
         ...
         %this.selectedImage++;
         if (%this.selectedImage > PixelPainter.NumberOfLevels)
            %this.selectedImage = PixelPainter.NumberOfLevels;
         else           
            %this.updateSelected();         
      }
      else
